...
researchers throughout the the data life cycle, the applicability of the
...
in methods and data as in ecology are facing problems in dealing with some
...
cleaning and merging and a exchange of datasets. 
* ich würde den Teilsatz über onotologies hier rausnehmen. Du weist ja am Ende des
Abstracts auf die kommenden features, wovon ontologies ein Teil sind....
We here introduce the `rbefata` package that connects the R statistic environment
to the open source data management platform `BEFdata` that has been developed
and is used within the BEF-China project.
...
databases of a certain domain and whose researchers get the
opportunity to access a wide range of relevant data all from one place. In fact, these
data management portals offer a solution to one of the most pressing
problems that we face with our valuable data today, their loss. However, by
developing tools which allow an easy access and analyses of these data it can
be ensured that these are not only preserved but used and reused. In this paper
we want to introduce the new R package rbefdata which linkes the befdata
portal with the statistics environment R. The aim of the package is to provide
the tools to download data, analyse them and upload them back to the data portal.

With a growing global data pool there is also a growing demand to use and
...
whose aim is to give the R - Users all the flexibility of data access through
...
project. It is a 15N tracer experiment which aims to disentangle the effect
...
BEF-China project.  The Biodiversity and Ecosystem
Functioning (BEF) China research group aims to disentangle the role of
tree and shrub diversity for production, erosion control, element
cycling, and species conservation in Chinese subtropical forest ecosystems.
It is funded by the German science foundation (DFG, FOR
891). The BEF-China research group (www.bef-china.de)
...
sampling design along tree species richness and stand age (cite Bruelheide, 2010).
...
heterogeneous data that usually has to be dealt with in BEF. However, its
...
In this paper we use already published datasets as a use case to present
...
al. 2013 (DOI: 10.1111/1365-2664.12157)
...
was significantly increased in mixtures.
For an interpretation of these results we refer to Lang et al. 2013.
