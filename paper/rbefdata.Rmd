...
### formal representation of knowledge to leverage data discovery:

To find relevant data in the growing pool of available data is a
challenge. While the extensive use of metadata (Metadata citations!) and the
tagging with keywords is a good start and in fact an essential part of the
solution. Nonetheless the process to decide on the relevance of data is still
very time and labour intensive. The integration of semantic resources like
thesauri and ontologies into data management and analysis tools offers a way
to help researchers not only to improve their search queries to find the
relevant data 



But ontology engineering and maintenance can be quite complex
for certain domains of knowledge that characterized by high heterogeneity
introduced through its interdisciplinary nature.


### Metadata is lost when loading .csv files to R platforms

The statistical environmant R is rapidly evolving and used by many scientists.
However, after data are found, they are mostly loaded into the R environment
as coma separated files and thus loose their metadata, including data
ownership information.

### sharing data and ideas through paper proposals
...
Workflows offer the potential to seamlessly trace research results back through the analysis to the original data, thus ensuring the reproducibility of science in data intensive research []. Workflow tools like Kepler, Pegasus []

However, as more and more scientists use the R statistical environment, there is no need to learn other software to access external data and knowledge sources. 
...
There is a need for tools that closely tie together  data discovery,
exploration, and management within the working environments of researchers. In
this paper we want to introduce the new R package `rbefdata`
(https://github.com/befdata/rbefdata) which links data repositories using the
open source data management platform `BEFdata`
(https://github.com/befdata/befdata) and Thematres thesauri (url thematres) to
the R statistics environment.  It provides tools to explore, download, and
upload  data from BEFdata repositories and at the same time explore and refine
keywords using Thematres thesauri. We showcase the functionality of the
...
depicts how to refine keywords for search in the thesaurus, ask for data
access in the data management platform, download data into the R environment,
inspect metadata, analyse the tracer experiment, and finally upload data and
workflow scripts back to the data management platform.
