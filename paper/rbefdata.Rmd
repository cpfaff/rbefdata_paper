...
statistics environment. We created an R package named `rbefdata` that provides
an access to the open source data management platform `BEFdata`. The R package
enables access to data stored on the platform as well as to the metadata
provided by dataset authors. It tightly integrates the metadata into R data
frames so it can be accessed directly when processing data from the portal.
`rbefdata` includes an access to an online thesaurus that can be used to
improve the discovery of datasets stored on the portal. Furthermore it provides
upload functionality so any data product can be pushed directly back to the
data portal for preservation. We integrate data management concepts like a
semantic improved exploration of datasets, a seamless access to data and
metadata and the preservation of data products directly into the R environment.
The open source character of the data management platform `BEFdata` and the
companion package `rbefata` makes it possible for any ecological project to use
this package as a sophisticated solution to their own data management
requirements. 
