...
data. The Ecological Metadata Language we use is only one example for that
(other are Geoscience Markup Language, Infrastructure for Spatial Information
in the European Community, NetCDF CF). While this theoretically solves the
problem with not well described datasets it is still hard to make researchers
use it extensively as this usually always means to learn new tools that help
with the description process (e.g morpho, data up).
...
other domains face more problems using it (cite xxx, morpho development team
announced semantic tagging but the plug-in did not appear anywhere). The
application of ontologies in ecology is discussed controversially (cite xxx)
and it is argued that they can be a huge benefit, but it is hard to set up a
sophisticated ontology covering all necessary terms and relation of a highly
complex research domains (cite xxx). 
...
data as well the reuse of available data. We showcase the functionality of the
...
data reuse and exchange (cite support as data intensive science). While data
storage and description is almost the same for all kind of data the effective
interlinking of data via an ontology requires the development of sound and
common accepted terminology all contributing scientist of a research domain use
but also help with the develop and discuss it. Thus collaborative ontology
...
also to develop and maintain them as community to keep them up date and
consistent over time. 

Thus we recently started to develop an ontology that by the time of writing is
based on terms drawn from projects that run an instance of the data management
platform BEFdata which are the BEF-China experiment (url) and the FUNdiv
project (url). We used a `tematres` server to set up and maintain the ontology
which contains by now 350 terms some of the with descriptions relevant for
biodiversity research. The `rbefdata` package will implement access to the
ontology by the recently published `rtematres` package which is also part of
the rOpenSci package portfolio. The `befdata` platform will get a semantical
tagging in the next upcoming version that will make it possible to tag each
column of a dataset



running with The software combination of `rbefdata` and `BEFdata` offers a
solutions to data storage as well as it helps in describing data with metadata. 

It also provides a public with the metadata as it is always accessible. With
more 

is used and the metadata is always
accessible. 

, it fosters the
collaboration and the trust in sharing data online by paper proposals and the
helps in analysing by offering simple tools to pull data right direct from the
portal into the R statistics environment. The upload functionality provided by
`rbefdata` 
...
This will be a short overview about other features of the `rbefdata` package as
only some core features have been used in the example.

* visualization plug-in for keywords

One can visualize the keywords associated with an instance of the  `BEFdata`
portal using the vitalization function `bef.portal.vizualize.keywords()`. This
gives a fast overview about what the data in a BEFdata portal is dealing with.
