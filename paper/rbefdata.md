...
developed and is used within the BEF-China experiment. It is specialized in
handling small and heterogeneous data, which is representing the majority of
data in biodiversity research. We show the usage of the package by creating a
workflow in form of an R script. The analysis steps underlying the workflow do
not only show the interactions between the R package and the data management
...
context of the data life cycle and the current as well as future data
