...
Today ecologists need to deal with a deluge of data that is generated by an
...
exploration and the reuse of data. There are many tools that provide solutions
to a wide range of problems in the life cycle of data. However not all of them
are widely spread or accepted throughout ecology. The acceptance of tools can
be improved by the tight integration of new and valuable data management
concepts into existing and widely used software and thus into the daily
workflow of researchers. Paper proposals and data provenance as new ways to
promote collaboration, metadata for data preservation and information exchange
as well as the integration of semantic resources, represent only a few examples
of concepts with the potential to improve the scientific progress. 
...
semantic repositories and smart merging functionality that will be implemented
in future versions of `rbefdata` and `BEFdata`.
