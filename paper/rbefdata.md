...
of them are widely spread or accepted throughout ecology. The accaptance can be
...
offers upload functionalities. We here use an ecological analysis about N
retention in the BEF-China experiment that has been published already to
showcase the functionality of the R package. The data used is open access and
thus the workflow in form of an R script presented here can be simply
reproduced. We discuss the introduced R package and the combination of software
in terms of the data life cycle and data management requirements in ecology.
...
merging functionality that will be implemented in future versions of `rbefdata`
and `BEFdata`.
