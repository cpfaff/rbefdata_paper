...
of which the results have been published already. The data used in the analysis
is open access so the workflow in this paper can be simply reproduced step by
step. We discuss the introduced combination of software in the terms of the
data life cycle and the current as well as future data management requirements.
...
* that is so redundant to the parts that follow isn't it? (why)
...
Workflows are a trending concept in terms of streamlining the process of data
access, processing and the preservation of valuable data products. The rOpenSci
initiative (http://ropensci.org/) is a community driven project to provide the
R statistic environment (cite R) with access to scientific data repositories
which enables the R users to keep their scripts and use the full flexibility of
data access from online repositories without the need to learn tool like Kepler
or any other graphical workflow solution.


While tools like Kepler () or Pegasus () require researchers to learn the new
tools th is a way to provide acknowledgement to data owners in a more granular
way. While special workflow tools like Kepler or Pegasus follow a graphical
approach to give researchers intuitive tools
