...
Today ecologists face a deluge of heterogeneous data generated by an
interdisciplinary research community. The reuse of data gets even more
important with more data getting available. This puts much pressure on the
development tools that help researchers to deal with the preservation, the
exploration as well as with the reuse of data. Many concepts and tools are
already available that provide solutions to different parts of the life cycle
of data. However not all of them are widely spread or accepted throughout
ecology. This situation could be improved by the tight integration of data
management concepts into existing and widely used tools and thus into the daily
workflow of researchers. Paper proposals and data provenance as new ways to
promote the discussion of ideas and collaboration, metadata for data
preservation and information exchange as well as the integration of semantic
resources represent only a few examples of highly valuable concepts with the
potential to improve the scientific progress. 
...
heterogeneous research data. The R package provides access to data and
metadata, integrates with a semantic repository and also offers upload
functionality for datasets and free format attachment files. 

To showcase the R package and the interactions with the `BEFdata` platform we
use an R script that pulls and analyses data from the BEF-China portal. The
analysis is about N retention and the data has been collected within BEF-China
experiment. As the results of the analysis are published already the data used
here is open access and thus the workflow presented can be simply reproduced.
We discuss the introduced R package and the combination of software in terms of
...
The
script reconstructs the analysis of a facet of an experiment within the
BEF-China experiment. As the analysis has been published already the data we
use here is open access.
