...
Another big problem with data, especially if we like to reuse available data,
...
While well described data helps in understanding datasets and on deciding upon
the relevance and applicability in a certain meta analysis there is still lots
of manual intervention necessary after that to prepare the data. It may needs
to be cleaned, imputed, reshaped and merged which usually takes up to 70% of
the analysis workflow before the (cite the workflow paper of Karin and me).
This step is not only time and labour intensive it is also potentially error
prone, especially as the complexity of analyses grows. 
...
* Here we provide one solution to the 
