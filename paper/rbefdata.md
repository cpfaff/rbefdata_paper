...
pressing problems like the loss of data other valuable concepts like for
example the use of meta data and formal representations of knowledge in form of
ontologies are not yet wide spread enough. This is either related to the fact
that the concepts are hard to implement for a certain domain or a lack of
acceptance. We here introduce the `rbefdata` package that links the R
...
between the R package and the data management platform but also reconstruct a
facet of an analysis where results have been published already. We discuss the
...
rbefdata is innovative in that it provides data together with metadata! This
should be introduced as a problem, that R dataframes are often restricted to
the tabular information, but we use the attributes to provide metadata
together with thabular data!
