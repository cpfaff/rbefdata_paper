...
development tools that help researchers with the preservation, the exploration
as well as with the reuse of data. Many tools and concepts available provide
solutions to different parts of the life cycle of data. However not all of them
are widely spread or accepted throughout ecology. This situation could be
improved by the tight integration of data management concepts into existing and
widely used software and thus into the daily workflow of researchers.  Paper
proposals and data provenance as new ways to promote the discussion of ideas
and the collaboration, metadata for data preservation and information exchange
as well as the integration of semantic resources represent only a few examples
of highly valuable concepts with the potential to improve the scientific
progress. 
...
functionality for datasets and free format attachment files. To showcase the R
package and the interactions with the `BEFdata` platform we use an R script
that pulls and analyses data from the `BEFdata` instance of the BEF-China
experiment. The analysis is about N retention and the data has been collected
within BEF-China experiment. As the results of the analysis are published
already the data used here is open access and thus the workflow presented can
be simply reproduced. We discuss the introduced R package and the combination
of software in terms of the data life cycle and the current data management
requirements in ecology and give an outlook on upcoming features of `rbefdata`
and `BEFdata`.
