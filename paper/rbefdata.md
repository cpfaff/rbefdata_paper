...
tools that not only help to explore but also to process, to reuse and to
prepare data for long term preservation. Although there is a rag rug of data
management tools and concepts already available that cover most of the data
management requirements in ecology, their tight integration into widely used
data analysis tools lacks behind. Thus we created an R package named `rbefdata`
that implements data management concepts and tools into the R statistics
environment. As companion package to the open source data management platform
`BEFdata` it offers a search for datasets that is improved by the integration
of an online thesaurus, the seamless access to datasets and metadata for
analysis and also provides upload functionality for data preservation. The open
source character of the data management platform `BEFdata` and the companion
package `rbefata` allow any ecological project to use this package as a
sophisticated solution to their own data management requirements. 

## Introduction 

* State the reason for the work
* the context and the hypotheses
* being tested. 
...
A growing amount of data available in ecology puts much pressure on the
development of tools that help researchers to deal with the exploration, the
reuse, the processing and the preservation of data products. Although there are
many valuable concepts and tools available that cover parts of the life cycle
of data, only a few of them are widely spread throughout ecology. This
situation is partly related to a missing tight integration of these data
management concepts into widely accepted tools like for example the R
statistics environment. 

The `rbefdata` R package together with the data management platform `BEFdata`
combine the strengths of the data management concepts mentioned into a
sophisticated data management software package that makes use of widely
accepted tools in ecology.

Paper proposals and data provenance as a way to promote collaborations by the
discussion of ideas and, metadata to improve data preservation and information
exchange as well as the integration of semantic resources to enhance data
exploration and processing, only represent a few examples of concepts with the
potential to improve the progress of a data intensive scientific domain like
ecology. 
...
range of relevant data all from one place. These platforms offer a solution to
one of the most pressing problems in data management, the loss of valuable data
(Heidorn, 2008). Even tough these platforms improve the situation in data
preservation, researchers are still reluctant to use online data platforms as
they fear to give away and loose the control over their data (michener 2012).
On top of that there is even no benefit in sharing data that would motivate to
put more time into a data to prepare it for preservation. The `BEFdata`
platform offers full control over the access to uploaded data via a fine
grained access system. Uploading data from within R via the `rbefdata` makes
the data accessible to the owner only by default but the access rights can be
adapted to working groups, project wide or even the public (cite Karin). 
...
standard. The R package provides the metadata of datasets as attributes of data
frames right from within R.
...
coarse as the search for one specific keyword may cover the access to a set of
data but misses other essential and relevant data tagged with close related or
synonymous keywords (Leinfelder et al. xxx, michener 2008). For example a
search with the keywords "weight" and "leaves" will likely miss datasets tagged
with "dry weight", "wet weight" or "plant organs", that might be associated to
datasets that are of interest as well. The integration of semantic repositories
that provide taxonomies of terms can improve the situation. These repositories
...
`rbefdata` package implements the access to semantic repositories stored on
`tematres` servers which can be used to improve the search query.
