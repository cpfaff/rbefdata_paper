...
do deal with the requirements that come with that. 

There are good solutions for data preservation but scientists are reluctant to
use them as they fear to give away and loose the control over their data. There
are good solutions to metadata as well but it is still hard to make scientists
use it extensively as it often requires to learn new tools. The situation could
potentially be improved by the development of software that tightly integrates
new valuable concepts into existing and widely used tools and thus into the
workflows of researchers. Paper proposals and metadata, data provenance as well
as well as the integration of semantic sources are only a few examples of
valuable concepts to be integrated. 
