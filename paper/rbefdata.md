...
Today ecologists face a deluge of heterogeneous data that is generated by an
interdisciplinary research community. This puts much pressure on the
development of tools that help with the preservation, the exploration as well
as with the reuse of data. Many tools and concepts available provide solutions
to different parts of the data life cycle. However not all of them are widely
spread or accepted throughout ecology. This situation could be improved by the
tight integration of data management concepts into existing and widely used
software and thus into the daily workflow of researchers. Paper proposals and
data provenance as new ways to promote the discussion of ideas and
collaborations, metadata for data preservation and information exchange as well
as the integration of semantic resources represent only a few examples of
highly valuable concepts with the potential to improve scientific progress. 
...
platform `BEFdata`. The R package provides access to data and metadata,
integrates with a semantic repository and also offers upload functionality to
preserve datasets and free format attachment files. To showcase the R package
and the interactions with the `BEFdata` platform we use an R script that pulls
and analyses data associated with a paper proposal on the `BEFdata` instance of
the BEF-China experiment. The results of the analysis that deals with soil N
retention and the role of species richness have been published already. The
data used is open access and thus the workflow script presented can be
reproduced by executing each step shown. We discuss the introduced R package
and the combination of software in terms of the data life cycle and the current
data management requirements in ecology and give an outlook on upcoming
...
ecology the reuse of data is of particular interest as the integration of data
offers the potential to answer questions on a much broader temporal and spatial
scale (cite xxx). A problem here is that available data often is not well
described. Metadata gives researchers the opportunity to understand raw
research data even if they are not familiar with it. It is necessary as plain
...
what the abbreviations and the headers in the dataset mean. However metadata is
still not used extensively as it usually means more to put more time into data
and to learn new tools that help to describe (e.g morpho, dataUP).

Finding relevant data for reuse is a challenging task and it is getting even
more complex when more data gets available. A common practice in data
discovery, next to full text search in metadata, is tagging with keywords.
However a keyword search can be very coarse as it provides access to a set of
data but misses other essential and relevant data. A keyword search for example
on "weight" of leaves will miss datasets tagged only with "dry weight" or "wet
...
data sharing (cite Karin). While metadata allows an insight into the actual raw
data without the need to give it away, paper proposals can serve serve as tool
to discuss ideas with data providers on analyses to be based on the data behind
the metadata. The integration of semantic repositories potentially improves not
only the discovery of data but also assists researchers with the preparation of
data on reuse. There is a growing demand on tools that integrate these concepts
into widely accepted and commonly used software. 

We here introduces the R package called `rbefdata` as companion package to the
open source data management platform `BEFdata` (cite Karin). The package is
part of the rOpenSci (http://ropensci.org/) initiative which is a community
...
to data and metadata stored on the data management platform. On top of that it
allows the use of a semantic repositories stored on `tematres` vocabulary
servers (http://www.vocabularyserver.com/) and provides upload functionality
for data as well as for free format attachments. 

The combination of `BEFdata` and `rbefdata` allow the automatization of common
analysis task which makes it workflow software. Workflows is a concept to
streamline and automate the processing of data. 


Workflow tools like Kepler and Pegasus integrate the access to data and
metadata, the manipulation and analysis of data as well as the preservation of


We showcase the functionality of the package available with version `0.3.5`.
The workflow reconstructs a facet of an analysis that has been published by
Lang et al. 2013 on nutrient retention along biodiversity gradients. It is a
15N tracer experiment which aims to disentangle the effect of species mixtures
on system N retention. The workflow depicts how to refine keywords for search
in the thesaurus, ask for data access in the data management platform, download
data into the R environment, inspect metadata, analyse the tracer experiment,
