...
integration of small datasets still lacks behind. This is partially related to
the reluctance of users towards new solutions, a general lack of acceptance of
certain concepts, or that some concepts are simply hard to implement for
certain scientific domains. We here introduce the `rbefdata` package that links
the R statistics environment to the open source data management platform
`BEFdata`.  The platform has been developed and is used within the BEF-China
experiment. It is specialized in handling small and heterogeneous data,
representing the majority of data in biodiversity research. We show the usage
of the package by a workflow in form of an R script. The analysis steps
underlying the workflow do not only show the interactions between the R package
and the data management platform but also reconstruct a facet of an analysis,
of which the results have been published already. We discuss the introduced
combination of software in the terms of the data life cycle and the current as
well as future data management requirements. Additionally, we give an outlook
on upcoming semantical assisted features like an improved exploration of data
and smart merging functionality to be integrated with `rbefdata` and `BEFdata`.
...
workflows of researchers to help them dealing with the challenges like data
...
workbench, GBIF, `BEFdata`, DataONE, LifeWatch). An ongoing trend in that
...
Simply finding relevant data for a certain analysis is crucial for an effective
reuse of available data. However this process is challenging and still a mostly
manual task. A common practice to data discovery is to tag datasets or data
columns with keywords that describe the content. The keywords can be used as
simple means to narrow down a set of relevant data. But a keyword search is
still very limiting without the using semantical repositories that embed a
keyword into a context. So a plain keywords search only help to find data,
tagged exactly with that term we are looking for. A search like this is highly
likely to miss other relevant data that is tagged with keywords that are close
related to the one we are looking for. Here the integration of formal
representations knowledge offers the potential to help or guide the process of
data discovery. For example a hierarchical representation of terms in the form
of a taxonomy can help in broaden or narrow down search terms to achieve a more
detailed or general search. Higher representations of formal knowledge like for
example ontologies can do even more as they allow formal reasoning so it allows
to develop tools that are smart enough to assist researchers on data reuse and
integration. While the development of a taxonomy is a more or less simple task
that is feasible even for smaller project the development of an ontology is
more challenging. It requires the input of many scientists and discussions
about definitions and relations of terms in the representation. This might be
one of the reasons why there is a lack of ontologies especially for highly
interdisciplinary research domains like ecology. 
...
Workflow software is a trending tool in terms of data processing as they not
only provide access to many data repository but also give researchers a more
visual way of communicating ongoing analyses.  However they also enables
researchers to seamlessly trace research results back through the analysis to
the original data that has been used. Thus ensuring the reproducibility of
science in data intensive research []. 

Like Kepler [] or Pegasus [] are 




However, as more and more scientists use the R statistical environment, there
is no need to learn other software to access external data and knowledge
sources. 

(http://ropensci.org/), which is a community driven approach to wrap all
science APIs and to create solutions for R users to seamlessly pull data from
different repositories spread over the internet into R for analysis.
...
easily integrate with their existing analysis workflows, enable them to simply
gain access to data repositories, assist them in discovery and the process of
understanding available data for an effective reuse. The process of assisted
discovery is highly likely to be enabled by the tight integration of semantic
repositories in form of taxonomies and ontologies in emerging software
solutions. 
...
(https://github.com/befdata/befdata) and enables a seamless access to data and
metadata stored on it. On top of that it integrates semantic repositories
stored on a `tematres` vocabulary server (http://www.vocabularyserver.com/).

The package provides tools to explore, download, and upload data from BEFdata
repositories and  

a way to refine keywords by the integration of the `tematres` vocabulary
server. 
...
analyses. The rationale of the paper proposal is:
